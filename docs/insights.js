...
// Set by the GitHub Pages copy (docs/insights.html): no server, practice data only.
const STATIC = window.SAVORA_STATIC === true;
...
async function load() {
  ...
  try {
    // On the GitHub Pages copy there is no server, so read a saved snapshot.
    const res  = await fetch(STATIC ? "insights-demo.json" : `/api/insights?${params}`);
...
if (STATIC) {
  // Snapshot copy: only practice data exists, and the other pages aren't hosted.
  document.querySelector(".toggle").style.display = "none";
  document.getElementById("kitchen-link").style.display = "none";
  const home = document.querySelector('.nav-link[href="/"]');
  if (home) { home.href = "./"; home.textContent = "ABOUT THE PROJECT"; }
  viewerChose = true;
  setSource("demo", false);
} else {
  let saved = null;
